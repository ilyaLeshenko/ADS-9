...
void RunExperiment() {
    ...
    for (int n = 1; n <= 8; ++n) {
        ...
        auto duration_perm1 = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();

        // Замер GetPerm2
        start = std::chrono::high_resolution_clock::now();
        auto perm2 = GetPerm2(tree, random_num);
        end = std::chrono::high_resolution_clock::now();
        auto duration_perm2 = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
